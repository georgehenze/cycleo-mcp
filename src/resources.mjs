...
export const RESOURCE_DEFINITIONS = [
  ...
  { uri: 'cycleo://rankings', name: 'cycleo_rankings', title: 'Cycleo league standings', description: 'The authenticated league standings (Cycleopunten, medal and special rankings).', mimeType: JSON_MIME },
  { uri: 'cycleo://transfers/history', name: 'cycleo_transfer_history', title: 'Cycleo transfer history', description: 'Completed transfer history for the authenticated league.', mimeType: JSON_MIME },
  { uri: 'cycleo://transfers/radar', name: 'cycleo_transfer_radar', title: 'Cycleo transfer radar', description: 'Frequently transferred riders for the authenticated league.', mimeType: JSON_MIME }
];
...
export const RESOURCE_TEMPLATES = [
  ...
  { uriTemplate: 'cycleo://races/{raceId}/result', name: 'cycleo_race_result_by_race', title: 'Cycleo race result by race', description: 'The authenticated league\'s calculated Cycleo result for a visible race.', mimeType: JSON_MIME },
  { uriTemplate: 'cycleo://races/{raceId}/classification', name: 'cycleo_race_classification_by_race', title: 'Cycleo race classification by race', description: 'The league-enriched final rider classification for a visible race.', mimeType: JSON_MIME },
...
export async function readResource(uri, { api, token, user }) {
  ...
  if ((match = /^cycleo:\/\/races\/([^/]+)\/result$/.exec(uri))) {
    return wrap(await api.get(`/races/${positiveId(match[1], 'raceId')}/cycleo-result`, token));
  }
  if ((match = /^cycleo:\/\/races\/([^/]+)\/classification$/.exec(uri))) {
    return wrap(await api.get(`/races/${positiveId(match[1], 'raceId')}/classification`, token));
  }
