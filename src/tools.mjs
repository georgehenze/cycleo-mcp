...
export const TOOL_DEFINITIONS = [
  { name: 'cycleo_get_current_time', title: 'Current time', description: 'Get the current time as an ISO 8601 UTC timestamp and Unix timestamp, plus local date/time in an optional IANA timezone. Defaults to the server timezone.', inputSchema: { type: 'object', properties: { timeZone: { type: 'string', minLength: 1, maxLength: 64, description: 'Optional IANA timezone, such as Europe/Amsterdam. Defaults to the server timezone.' } }, additionalProperties: false } },
  ...
  { name: 'cycleo_list_races', title: 'List Cycleo races', description: 'List race editions for a season, optionally filtered by status and paging. Defaults to the current season and all statuses.', inputSchema: { type: 'object', properties: { year: { type: 'integer', minimum: 2000, maximum: 2100 }, status: { type: 'string', enum: ['all', 'upcoming', 'active', 'completed', 'cancelled'], default: 'all' }, limit: { type: 'integer', minimum: 1, maximum: MAX_LIMIT }, page: { type: 'integer', minimum: 1 } }, additionalProperties: false } },
  ...
  { name: 'cycleo_get_rider_startlist_races', title: 'Rider start-list races', description: 'List every current or upcoming not-yet-calculated Cycleo race whose start list includes this rider. If only a rider name is known, use cycleo_search_entities with entityType=rider and pass the matched rider id as riderId.', inputSchema: { type: 'object', required: ['riderId'], properties: { riderId: { type: 'integer', minimum: 1 } }, additionalProperties: false } },
  ...
  { name: 'cycleo_get_race_startlist', title: 'Cycleo race start list', description: 'Get all riders on a race edition start list, including bibs, withdrawal status, professional teams, and ownership in the authenticated user\'s league. If only a race name is known, use cycleo_search_entities with entityType=race and pass the matched editionId as raceId.', inputSchema: { type: 'object', required: ['raceId'], properties: { raceId: { type: 'integer', minimum: 1 } }, additionalProperties: false } },
...
function currentTime(requestedTimeZone = SERVER_TIME_ZONE, now = new Date()) {
  const timeZone = requestedTimeZone.trim() || SERVER_TIME_ZONE;
  let dateFormatter;
  let timeFormatter;
  try {
    ...
    if (error instanceof RangeError) throw invalidArguments('timeZone must be a valid IANA timezone');
    throw error;
  }
  const resolvedTimeZone = dateFormatter.resolvedOptions().timeZone;
  const dateParts = Object.fromEntries(dateFormatter.formatToParts(now).filter(({ type }) => type !== 'literal').map(({ type, value }) => [type, value]));
  const timeParts = Object.fromEntries(timeFormatter.formatToParts(now).filter(({ type }) => type !== 'literal').map(({ type, value }) => [type, value]));
...
export async function callTool(name, args, { api, token, user }) {
  ...
  switch (name) {
    case 'cycleo_get_current_time': return currentTime(args.timeZone?.trim() || SERVER_TIME_ZONE);
    ...
    case 'cycleo_list_races': return api.get('/races', token, { year: args.year, status: args.status, page: integer(args.page), limit: limit(args.limit) });
    ...
    case 'cycleo_get_race_startlist': return api.get(`/races/${integer(args.raceId)}/startlist`, token);
